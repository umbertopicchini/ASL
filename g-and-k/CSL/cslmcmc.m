function MCMC= bslmcmc(thetastart,data,numsim,R_mcmc,cov_current,burnin,length_CoVupdate,numgroups,mcwm)
...
for mcmc_iter = 3:burnin/2 
   ...
   elseif mcwm==1 % no correlation is applied
       z_gauss = randn(nobs,numsim);
       loglik = gk_synlik(theta,sobs,numsim,z_gauss);
       loglik_old = gk_synlik(theta_old,sobs,numsim,randn(nobs,numsim));
   end
   % evaluate priors at proposed parameters
   prior =  gk_prior(theta);
