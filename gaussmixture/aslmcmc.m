function [MCMC,proposal_cov]= aslmcmc(thetastart,data,numsim,R_mcmc,step_rw,length_CoVupdate,burnin,randomwalk_iterstart,verbose,mcwm)
...
sobs = mixture_sumstat(data,1)     % summary statistics of data
dtheta = length(thetastart);
dsobs = length(sobs);
%:::::::::::::::::::::::::::::: INITIALIZATION  ::::::::::::::::

[loglik_old,~,cov_simsummaries,simsum] = mixture_synlik(theta_old,sobs,nobs,numsim);
...
prior_old =  mixture_prior(theta_old);
%:::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::
...
[loglik,~,cov_simsummaries,simsum] = mixture_synlik(theta,sobs,nobs,numsim);

%:::::::::::::::::::::::::::::: PRIORS :::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::
% evaluate priors at proposed parameters
prior =  mixture_prior(theta);
%:::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::
...
for mcmc_iter = 3:burnin/2 
   ...
   [loglik,~,cov_simsummaries,simsum] = mixture_synlik(theta,sobs,nobs,numsim);
   if mcwm
       [loglik_old,~,~,~] = mixture_synlik(theta_old,sobs,nobs,numsim);
   end
 %  loglik
   % evaluate priors at proposed parameters
   prior =  mixture_prior(theta);
...
%        all_cov_thetasimsum = cov(accepted_thetasimsum(1:mcmc_iter,:));
   ...
   [loglik,~,cov_simsummaries,simsum] = mixture_synlik(theta,sobs,nobs,numsim);
   if mcwm  % "refresh" the old likelihood during burnin only (perform Markov chain Within Metropolis)
       if mcmc_iter <= burnin
          [loglik_old,~,~,~] = mixture_synlik(theta_old,sobs,nobs,numsim);
       end
   end
