function [loglik,mean_simsummaries,cov_simsummaries,simsummaries] = alphastable_synlik(thetatilde,sobs,covariates,nobs,numsim,unifdraws)
...
% the case alpha != 1
W = -log(unifdraws(1:nobs,:));  % produces draws from the Exponential(lambda=1) distribution
U = unifdraws(nobs+1:end,:)*pi -pi/2; % produces draws from the Unif(-pi/2,pi/2) distribution
